...
export const opportunityFilters: Array<'Todos' | OpportunityType> = ['Todos', 'Sem site', 'Site desatualizado', 'Perfil incompleto']
  ...
  return (
    <div className="filter-tabs" role="tablist" aria-label="Filtrar oportunidades">
      {opportunityFilters.map((filter) => (
        <button key={filter} className={activeFilter === filter ? 'selected' : ''} onClick={() => onFilterChange(filter)} type="button" role="tab" aria-selected={activeFilter === filter}>
          {filter}
        </button>
      ))}
    </div>
