...
export function searchLeads(leads: Lead[], query: SearchLeadsQuery): Lead[] {
  ...
  const segmentTerms = segment.split(/,|\se\s/).map((term) => term.trim()).filter((term) => term.length > 3)

  return leads.filter((lead) => {
    const normalizedCategory = normalize(lead.category)
    const matchesCity = normalize(lead.address).includes(cityName)
    const matchesSegment = !segment || segment === 'todos os segmentos' || segmentTerms.some((term) => normalizedCategory.includes(term) || term.includes(normalizedCategory))
